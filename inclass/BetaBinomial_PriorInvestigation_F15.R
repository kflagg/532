...
#        #
#        # Simulate 1000 draws of phi1 and phi2 from marginal posterior
#        #
#        pos.1=sample(10000,1000,prob=exp(logpost+733.8237),rep=T)
#        phi1.samp=rep.phi1[pos.1]
#        phi2.samp=rep.phi2[pos.1]
#        #
#        # Transform the draws to the (alpha,beta) scale
#        #
#        beta.sample=exp(phi2.samp)/(1+exp(phi1.samp))
#        alpha.sample=beta.sample*exp(phi1.samp)
#        #
#        # Sample theta from the conditional posterior
#        #
#        theta.sample=rbeta(1000,alpha.sample+tumor.y[1],beta.sample+tumor.n[1]-tumor.y[1])
#        for(i in 2:71){
#          theta.sample=cbind(theta.sample,rbeta(1000,alpha.sample+tumor.y[i],
#                                                beta.sample+tumor.n[i]-tumor.y[i]))}
...
curve(var.beta(x,b=0.01), from=0, to=1, col=2,)
