...
#[3,] -0.0030123256  0.051382611  0.099574752
  ...
  ### FIRST USE A DIAGONAL VAR-COV MATRIX for the PROPOSAL DISTN
   ...
   for (i in 2:nsim) {
     ...
     phi.cand1 <- rmvnorm(1, mean=center, sigma=2*ind.varcov)
     phi.cand2 <- rmvnorm(1, mean=center, sigma=2*ind.varcov)
     phi.cand3 <- rmvnorm(1, mean=center, sigma=2*ind.varcov)
   ...
   for (i in 2:nsim) {
     ...
     phi.cur2 <- phi.mat2[i-1,]
     phi.cur3 <- phi.mat3[i-1,]
     phi.cand1 <- rmvnorm(1, mean=center, sigma=2*VarCov)
     phi.cand2 <- rmvnorm(1, mean=center, sigma=2*VarCov)
     phi.cand3 <- rmvnorm(1, mean=center, sigma=2*VarCov)
