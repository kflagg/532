...
##   (See Figure 5.9)
 ...
 #setwd("/Users/meganhiggs/Documents/STAT 532 - Bayesian Fall2015/Hierarchical models")
  ...
  #Summarize the draws, make plots in text
   ...
   ###   school A effect is as large as 28 points:
    ...
    ## Note - these are NOT posterior predictive distributions
      ...
      par(mfrow=c(1,2))
      hist(max.effect, col=gray(.5), nclass=100, freq=FALSE, main="max effect- folded-t", xlim=c(-20,70))
       abline(v=28, col="orange", lwd=2)
      hist(max.effect2, col=gray(.5), nclass=100, freq=FALSE, main="max effect- unif", xlim=c(-20,70))
        abline(v=28, col="orange", lwd=2)
